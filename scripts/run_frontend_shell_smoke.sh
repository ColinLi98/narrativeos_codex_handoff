...
SMOKE_DB="${ROOT_DIR}/artifacts/frontend_shell_smoke.db"
RESULT_FILE="${ROOT_DIR}/artifacts/frontend_shell_smoke_result.json"
FAILURE_ARTIFACT_FILE="${ROOT_DIR}/artifacts/frontend_shell_smoke_failure_snapshot.json"
FAILURE_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/frontend_shell_smoke_failure.png"
APP_PORT="${APP_PORT:-8010}"
APP_URL="${APP_URL:-http://127.0.0.1:${APP_PORT}/app?debug=1}"
CHROME_PORT="${CHROME_PORT:-9224}"
CHROME_USER_DIR="${CHROME_USER_DIR:-/tmp/narrativeos-chrome-frontend-shell}"
CHROME_APP="${CHROME_APP:-/Applications/Google Chrome.app}"
CHROME_BIN="${CHROME_BIN:-}"
CHROME_EXTRA_ARGS="${CHROME_EXTRA_ARGS:-}"
CI_HEADLESS="${CI_HEADLESS:-${CI:-}}"
SERVER_LOG="${SERVER_LOG:-/tmp/frontend_shell_smoke_server.log}"
CHROME_LOG="${CHROME_LOG:-/tmp/frontend_shell_smoke_chrome.log}"
SERVER_PID=""
CHROME_PID=""
...
if curl -sf "http://127.0.0.1:${APP_PORT}/health" >/dev/null 2>&1; then
  echo "Reusing existing NarrativeOS API on port ${APP_PORT}..."
...
echo "Running frontend shell smoke verification..."
node "${ROOT_DIR}/scripts/verify_frontend_shell_smoke.js" \
  --url "${APP_URL}" \
...
echo "Frontend shell smoke passed."
