...
SQL_PATH = ROOT_DIR / "db" / "migrations" / "0016_grounding_checks.sql"
...
def downgrade() -> None:
    bind = op.get_bind()
    for statement in [
        "drop index if exists idx_grounding_checks_session_created_at",
        "drop index if exists idx_grounding_checks_world_created_at",
        "drop index if exists idx_grounding_checks_status_created_at",
        "drop index if exists idx_grounding_checks_trace_created_at",
        "drop table if exists grounding_checks",
    ]:
        bind.exec_driver_sql(statement)
    bind.exec_driver_sql(
        f"""
