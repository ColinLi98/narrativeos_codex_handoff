...
VERIFICATION_TABLES = [
    ...
    ("plans", PlanRow),
    ("customer_accounts", CustomerAccountRow),
    ("billing_profiles", BillingProfileRow),
    ("usage_ledgers", UsageLedgerRow),
    ("billable_events", BillableEventRow),
    ("invoice_previews", InvoicePreviewRow),
    ("credit_balances", CreditBalanceRow),
    ("overage_flags", OverageFlagRow),
    ("campaigns", CampaignRow),
    ("campaign_proof_bundles", CampaignProofBundleRow),
    ("campaign_channel_targets", CampaignChannelTargetRow),
    ("campaign_review_submissions", CampaignReviewSubmissionRow),
    ("partners", PartnerRow),
    ("partner_capabilities", PartnerCapabilityRow),
    ("partner_health_checks", PartnerHealthCheckRow),
    ("disputes", DisputeRow),
    ("refund_requests", RefundRequestRow),
    ("settlement_runs", SettlementRunRow),
    ("settlement_items", SettlementItemRow),
    ("support_cases", SupportCaseRow),
    ("manual_adjustments", ManualAdjustmentRow),
    ("audit_logs", AuditLogRow),
    ("customer_audit_exports", CustomerAuditExportRow),
    ("data_retention_policies", DataRetentionPolicyRow),
    ("data_deletion_requests", DataDeletionRequestRow),
    ("invoice_issuances", InvoiceIssuanceRow),
    ("payment_transactions", PaymentTransactionRow),
    ("provider_webhook_events", ProviderWebhookEventRow),
    ("credit_notes", CreditNoteRow),
    ("payment_retry_attempts", PaymentRetryAttemptRow),
    ("dunning_events", DunningEventRow),
    ("renewal_trackers", RenewalTrackerRow),
    ("dunning_runs", DunningRunRow),
    ("pilot_conversion_tracks", PilotConversionTrackRow),
    ("expansion_candidates", ExpansionCandidateRow),
    ("churn_risk_flags", ChurnRiskFlagRow),
    ("production_signoffs", ProductionSignoffRow),
    ("production_signoff_items", ProductionSignoffItemRow),
    ("production_signoff_evidence", ProductionSignoffEvidenceRow),
    ("production_cutover_windows", ProductionCutoverWindowRow),
    ("production_customer_acceptance_records", ProductionCustomerAcceptanceRecordRow),
    ("go_live_ready_accounts", GoLiveReadyAccountRow),
    ("launch_wave_statuses", LaunchWaveStatusRow),
    ("production_preflight_runs", ProductionPreflightRunRow),
    ("production_preflight_checks", ProductionPreflightCheckRow),
    ("first_7_day_outcomes", First7DayOutcomeRow),
    ("first_30_day_value_summaries", First30DayValueSummaryRow),
    ("pilot_to_paid_readiness_scores", PilotToPaidReadinessScoreRow),
    ("customer_success_snapshots", CustomerSuccessSnapshotRow),
    ("library_stats_cubes", LibraryStatsCubeRow),
    ("production_launch_events", ProductionLaunchEventRow),
    ("production_postmortem_records", ProductionPostmortemRecordRow),
    ("go_live_day_runs", GoLiveDayRunRow),
    ("go_live_day_checkpoints", GoLiveDayCheckpointRow),
...
class RuntimeOpsService:
    ...
    def build_preflight_verification_bundle(self, *, account_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        verification_commands = [
            ...
            "bash scripts/run_commercial_long_route_50.sh "
            "# emits artifacts/commercial_long_route_50.json and artifacts/commercial_long_route_50.md",
            "cd Kimi_Agent_设计系统加载/app && npm run smoke:local "
            "# emits artifacts/quantum_local_acceptance_smoke_result.json with fixed ports 3000/8000",
            "cd Kimi_Agent_设计系统加载/app && npm run smoke:reader-paid "
            "# emits artifacts/reader_paid_path_smoke_result.json with fixed ports 3000/8000",
            "bash scripts/run_author_repair_loop_smoke.sh "
            "# emits artifacts/author_repair_loop_smoke_result.json for brief -> draft -> asset edit -> simulate repair-loop evidence",
            "bash scripts/run_stripe_sandbox_local.sh "
            "# emits artifacts/stripe_external_acceptance/latest/external_acceptance_summary.json",
            "python scripts/run_backup_restore_verification_hooks.py "
            "# emits backup/restore verification hook evidence for production preflight",
            "bash scripts/run_quantum_ops_url_state_smoke.sh "
            "# emits artifacts/quantum_ops_url_state_smoke_result.json for Ops alerts/governance URL-state evidence",
            "python scripts/build_operator_evidence_closure.py "
            "# emits artifacts/human_signoff_closure/latest/operator_evidence_closure.json for production operator evidence closure",
            "bash scripts/run_cross_pack_merge_gate.sh",
        ]
        return {
            "generated_at": self._utcnow(),
