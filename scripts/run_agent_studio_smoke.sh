...
SMOKE_DB="${ROOT_DIR}/artifacts/agent_studio_smoke.db"
RESULT_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_result.json"
FAILURE_ARTIFACT_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_failure_snapshot.json"
FAILURE_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_failure.png"
DESKTOP_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_desktop.png"
MOBILE_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_mobile.png"
VISUAL_REVIEW_FILE="${ROOT_DIR}/artifacts/agent_studio_smoke_visual_review.md"
APP_PORT="${APP_PORT:-8018}"
APP_URL="${APP_URL:-http://127.0.0.1:${APP_PORT}/app?product=author&workspace=studio&debug=1}"
CHROME_PORT="${CHROME_PORT:-9238}"
CHROME_USER_DIR="${CHROME_USER_DIR:-/tmp/narrativeos-chrome-agent-studio}"
CHROME_APP="${CHROME_APP:-/Applications/Google Chrome.app}"
CHROME_BIN="${CHROME_BIN:-}"
CHROME_EXTRA_ARGS="${CHROME_EXTRA_ARGS:-}"
CI_HEADLESS="${CI_HEADLESS:-${CI:-}}"
SERVER_LOG="${SERVER_LOG:-/tmp/agent_studio_smoke_server.log}"
CHROME_LOG="${CHROME_LOG:-/tmp/agent_studio_smoke_chrome.log}"
SERVER_PID=""
CHROME_PID=""
...
rm -f "${SMOKE_DB}" "${RESULT_FILE}" "${FAILURE_ARTIFACT_FILE}" "${FAILURE_SCREENSHOT_FILE}" "${DESKTOP_SCREENSHOT_FILE}" "${MOBILE_SCREENSHOT_FILE}" "${VISUAL_REVIEW_FILE}"
...
echo "Running Agent Studio smoke verification..."
node "${ROOT_DIR}/scripts/verify_agent_studio_smoke.js" \
  --url "${APP_URL}" \
  --database-url "${DATABASE_URL}" \
  --result-file "${RESULT_FILE}" \
  --failure-artifact-file "${FAILURE_ARTIFACT_FILE}" \
  --failure-screenshot-file "${FAILURE_SCREENSHOT_FILE}" \
  --desktop-screenshot-file "${DESKTOP_SCREENSHOT_FILE}" \
  --mobile-screenshot-file "${MOBILE_SCREENSHOT_FILE}" \
  --visual-review-file "${VISUAL_REVIEW_FILE}" \
  --chrome-port "${CHROME_PORT}"

echo "Agent Studio smoke passed."
