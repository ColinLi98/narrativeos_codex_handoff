...
SMOKE_DB="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke.db"
SEED_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_seed.json"
RESULT_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_result.json"
HISTORY_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_history.json"
FAILURE_ARTIFACT_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_failure_snapshot.json"
FAILURE_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_failure.png"
STORYBOOK_SCREENSHOT_FILE="${ROOT_DIR}/artifacts/reader_storybook_long_route_smoke_storybook.png"
APP_PORT="${APP_PORT:-8011}"
APP_URL="${APP_URL:-http://127.0.0.1:${APP_PORT}/app?debug=1}"
CHROME_PORT="${CHROME_PORT:-9225}"
CHROME_USER_DIR="${CHROME_USER_DIR:-/tmp/narrativeos-chrome-reader-storybook-long-route}"
CHROME_APP="${CHROME_APP:-/Applications/Google Chrome.app}"
CHROME_BIN="${CHROME_BIN:-}"
CHROME_EXTRA_ARGS="${CHROME_EXTRA_ARGS:-}"
CI_HEADLESS="${CI_HEADLESS:-${CI:-}}"
WORLD_IDS="${WORLD_IDS:-jade_court_exam,jade_court_romance,urban_mystery_lotus_lane}"
TARGET_CHAPTERS="${TARGET_CHAPTERS:-30}"
MIN_TARGET_CHAPTERS="${MIN_TARGET_CHAPTERS:-30}"
MAX_ATTEMPTS_PER_CHAPTER="${MAX_ATTEMPTS_PER_CHAPTER:-4}"
SERVER_LOG="${SERVER_LOG:-/tmp/reader_storybook_long_route_smoke_server.log}"
CHROME_LOG="${CHROME_LOG:-/tmp/reader_storybook_long_route_smoke_chrome.log}"
SERVER_PID=""
CHROME_PID=""
...
rm -f "${SMOKE_DB}" "${SEED_FILE}" "${RESULT_FILE}" "${FAILURE_ARTIFACT_FILE}" "${FAILURE_SCREENSHOT_FILE}" "${STORYBOOK_SCREENSHOT_FILE}"
rm -rf "${CHROME_USER_DIR}"
rm -f "${SERVER_LOG}" "${CHROME_LOG}"

DATABASE_URL="sqlite:///${SMOKE_DB}"

echo "Seeding reader storybook long-route dataset..."
"${VENV_PYTHON}" "${ROOT_DIR}/scripts/seed_reader_storybook_long_route_smoke.py" \
...
echo "Running Reader storybook long-route smoke verification..."
node "${ROOT_DIR}/scripts/verify_reader_storybook_long_route_smoke.js" \
  --url "${APP_URL}" \
  --seed-file "${SEED_FILE}" \
  ...
  --storybook-screenshot-file "${STORYBOOK_SCREENSHOT_FILE}" \
  --chrome-port "${CHROME_PORT}"

echo "Reader storybook long-route smoke passed."
